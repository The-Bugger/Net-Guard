"""
Cross-platform firewall abstraction for NetGuard.

Selects the backend at import time:
  - Linux  → iptables / ip6tables
  - Windows → netsh advfirewall

Public API (all return bool):
    fw_block(ip)   — add DROP/block rule
    fw_unblock(ip) — remove rule
    fw_check()     — verify privileges / tool availability

ponytail: Windows netsh rules are named "NetGuard-block-<ip>" for reliable removal.
IPv6 on Windows uses the same netsh command family.
Ceiling: netsh doesn't support CIDR ranges natively; country/ASN blocks on
Windows remain DB-only.
"""
...
def _run(cmd: list[str], timeout: int = 5) -> bool:
    """Run cmd, return True on rc=0. Never raises."""
    try:
...
def _rule_name(ip: str) -> str:
    """Windows firewall rule name for an IP (colons replaced with hyphens for IPv6)."""
...
def _win_unblock(ip: str) -> bool:
    name = _rule_name(ip)
    return _run(["netsh", "advfirewall", "firewall", "delete", "rule", f"name={name}"])
