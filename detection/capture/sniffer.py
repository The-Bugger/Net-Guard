"""
Packet capture engine for NetGuard IDPS.

Opens a network interface via Scapy and forwards decoded packets to the
detection pipeline through a thread-safe queue. Falls back to simulation
mode when Scapy/libpcap is unavailable (e.g. Windows without Npcap).
"""
...
# Check Scapy availability at import time. The runtime probe in _capture_loop
# confirms actual usability (sniff() can fail even when the import succeeds).
...
class CaptureEngine:
    ...
    def __init__(self, packet_queue: queue.Queue, socketio_emit=None, stats_service=None) -> None:
    ...
    def start(self, interface: str) -> None:
        ...
        # Use a real OS thread so blocking Scapy/pcap calls don't stall the eventlet pool.
        import threading as _threading
        self._thread = _threading.Thread(target=self._capture_loop, name="Packet_Capture_Thread", daemon=True)
        self._thread.start()
    ...
    def _on_packet(self, raw_pkt) -> None:
        """Scapy callback for every captured packet."""
        try:
            decoded = self._decoder.decode(raw_pkt)
            if decoded is None:
                _system_logger.warning("CaptureEngine: packet could not be decoded — discarding malformed packet.")
                return
    ...
    def _capture_loop(self) -> None:
        """Try real Scapy capture; fall back to simulation if libpcap is unavailable."""
        import sys as _sys

        if not _SCAPY_AVAILABLE:
            logger.warning("CaptureEngine: Scapy unavailable — SIMULATION MODE on '%s'.", self._interface)
            self._emit_sim_status()
            self._simulation_loop()
            return

        # On Windows, sniff() can block even with timeout= when Npcap is absent.
        # Attempt live capture directly and fall back to sim on failure.
        if _sys.platform == "win32":
            try:
                from scapy.sendrecv import sniff
                logger.info("CaptureEngine: live capture starting on '%s' (Windows).", self._interface)
                while not self._stop_event.is_set():
                    sniff(iface=self._interface, prn=self._on_packet, store=False, timeout=0.5, count=0)
                return
            except Exception as exc:
                logger.warning("CaptureEngine: live capture failed on '%s' (%s) — SIMULATION MODE.", self._interface, exc)
                self._emit_sim_status()
                self._simulation_loop()
                return
        ...
        try:
            ...
            # Runtime probe in a separate thread to avoid blocking on systems
            # where sniff() hangs even with timeout= set.
            probe_ok = [False]
            probe_exc_holder = [None]

            def _probe():
                try:
                    sniff(iface=self._interface, store=False, timeout=0.1, count=1, stop_filter=lambda _: True)
                    probe_ok[0] = True
                except Exception as exc:
            ...
            if not probe_ok[0]:
                ...
                logger.warning("CaptureEngine: probe failed on '%s' (%s) — SIMULATION MODE.", self._interface, exc_info)
                self._emit_sim_status()
                self._simulation_loop()
                return
            ...
            logger.warning("CaptureEngine: falling back to SIMULATION MODE after capture error.")
            self._emit_sim_status()
        ...
        if self._socketio_emit:
            try:
                self._socketio_emit("monitoring_status", {
                    "active": True, "interface": self._interface, "mode": "simulation",
                })
            except Exception:
                pass
    ...
    def _simulation_loop(self) -> None:
        """
        Generate synthetic packets when libpcap is unavailable.

        Starts with a 20–30 s quiet period of normal traffic, then fires
        2–3 attack bursts per minute at randomised intervals.
        ...
        ATTACKER_IPS_ASIA = [
            "103.41.167.21", "182.72.180.1", "27.251.16.1", "49.36.187.45", "115.112.82.1",
            "203.0.113.99", "116.228.101.1", "183.2.172.1",
            "103.92.45.1", "103.168.206.1",
            "39.32.100.1", "202.83.24.1",
            "203.0.113.200", "103.77.4.82", "14.225.196.1", "49.231.100.1", "118.189.149.1",
        ...
        # Quiet startup: 20–30 s of normal traffic before first attack
        ...
        while not self._stop_event.is_set():
            ...
            if now >= next_attack:
                ...
                if random.random() < 0.10:
                    next_attack = now + random.uniform(10, 18)   # burst mode
    ...
    def _sim_send_normal(self, attacker_ips, local_ips, ports):
        ...
        for _ in range(random.randint(1, 3)):
            if self._stop_event.is_set():
                break
            src_pool = local_ips + attacker_ips[:2] if len(attacker_ips) >= 2 else local_ips + attacker_ips
            pkt = Packet(
