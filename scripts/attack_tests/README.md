...
Running all five scripts back-to-back produces the following sequence on the NetGuard dashboard:
...
Health score after all five: 20/100 (all five deductions + multi-attack penalty)
```

After the run, `GET /api/v1/events` should return ≥ 5 rows each with a distinct `attack_type`.
